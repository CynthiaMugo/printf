...
{
	/* char is stored on the left and padding at the buffers right */
	int i = 0;
	char padd = ' ';
 ...
 * @flags: Calculates number of flags
 * @width: get the width
...

{
	...
	if (prec == 0 && ind == BUFF_SIZE - 2 && buffer[ind] == '0')
		buffer[ind] = padd = ' ';
	/* width is displaying with padding ' ' */
	...
	if (width > length)
	{
		...
		{
			/* extra char to the left of buff */
			if (extra_c)
 ...
 * @is_negative: indicates whether a number is negative
...
int write_unsgnd(int is_negative, int ind, char buffer[],
		int flags, int width, int precision, int size)
{
	/* number stored at buffer's right and starts at position i */
